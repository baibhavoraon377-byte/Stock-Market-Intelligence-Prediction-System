...
with st.sidebar:
    ...
    <div style="padding:1.2rem .4rem .4rem">
        ...
        <div class="nav-item">Risk Metrics</div>
    ...
    if custom_sym.strip():
        stock_symbol = custom_sym.strip().upper()

    period = st.selectbox("Period", ["1mo", "3mo", "6mo", "1y", "2y", "5y"], index=2)

    # Compare-with multiselect using full names
    cmp_names    = st.multiselect(
        "Compare With",
        stock_names,
        default=["Apple (AAPL)", "Microsoft (MSFT)"],
    )
    compare_stocks = [STOCK_CATALOGUE[n] for n in cmp_names]
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["Technical", "Risk", "Correlation", "Price Levels", "AI Signals"]
)
