"""
Advanced Analytics + ML Prediction + Portfolio Benchmark vs S&P 500
...
def calc_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Target: next-5-day return positive?
    df["target"] = (df["Close"].shift(-5) > df["Close"]).astype(int)
    return df

FEATURES = ["ret1","ret5","ret20","RSI","MACD_diff","BB_pos","Stoch","ATR",
            "vol20","MA_ratio","OBV_ret"]
...
with st.sidebar:
    ...
    ml_model = st.selectbox("ML Model",["Random Forest","Gradient Boost"]+["XGBoost"]*HAS_XGB)

# ── Header ─────────────────────────────────────────────────────────────────────
st.markdown(f'<div style="font-family:\'Syne\',sans-serif;font-size:1.6rem;font-weight:800;color:#f0f2f8;padding:.6rem 0 .2rem">Advanced Analytics — {symbol}</div><div style="font-size:.82rem;color:#8892a4">Technical analysis · Risk metrics · Real ML predictions · Benchmark comparison</div>', unsafe_allow_html=True)
...
tabs = st.tabs(["📈 Technical","⚠️ Risk","🤖 ML Prediction","📊 Benchmark","🎯 Price Levels","🔗 Correlation"])
...
with tab2:
    ...
    for col,lbl,val in [(c4,"Max Drawdown",f"{risk['Max Drawdown']:.1f}%"),
    ...
    st.markdown(f'<div class="insight-text" style="margin-top:.7rem"><strong style="color:#f0f2f8">Risk Level: <span style="color:{rc_}">{rl}</span></strong> · Score {rs}/100<br>{"⚠️ High vol — size positions carefully." if rl=="High" else "📊 Moderate — typical growth profile." if rl=="Medium" else "✅ Conservative risk profile."}</div>',unsafe_allow_html=True)
...
# ── ML PREDICTION ─────────────────────────────────────────────────────────────
with tab3:
    st.markdown(f'<div style="font-family:\'Syne\',sans-serif;font-size:.9rem;font-weight:700;color:#f0f2f8;margin-bottom:.4rem">Real ML Price Direction Prediction — {ml_model}</div>',unsafe_allow_html=True)
    ...
    st.markdown('<div class="insight-text"><strong style="color:#f0f2f8">⚠️ Disclaimer</strong><br>ML models are trained on historical patterns. Predictions are probabilistic and do not constitute financial advice. Past accuracy does not guarantee future results.</div>',unsafe_allow_html=True)
...
with tab4:
    ...
    if df_spy is None:
        ...
        st.markdown(f'<div class="insight-text" style="margin-top:.6rem"><strong style="color:#f0f2f8">Interpretation</strong><br>Beta of <strong>{beta:.2f}</strong> means {symbol} moves roughly {beta:.1f}× the {bench}. Alpha of <strong>{alpha_ann:+.1f}%</strong>/yr represents {"outperformance ✅" if alpha_ann>0 else "underperformance ⚠️"}. Correlation: <strong>{corr:.2f}</strong>.</div>',unsafe_allow_html=True)
        ...
        with pd.ExcelWriter(buf,engine="openpyxl") as w:
            norm.to_excel(w,sheet_name="Performance")
            rets.to_excel(w,sheet_name="Returns")
        st.download_button("📥 Export Report (Excel)",buf.getvalue(),
...
with tab5:
    ...
    fibs = {"23.6%":hi-diff*.236,"38.2%":hi-diff*.382,"50.0%":hi-diff*.5,"61.8%":hi-diff*.618}
    ...
    <tr><td>Price Position</td><td>{pos:.1f}% — {"Near Resistance ⚠️" if pos>70 else "Near Support 🟢" if pos<30 else "Mid Range"}</td></tr>
    </table></div>""",unsafe_allow_html=True)
        ...
        for s in compare:
            if s in corr_df.columns:
                ...
                meaning = "highly correlated (low diversification)" if cv>.7 else \
                          "negatively correlated (good diversification)" if cv<-.3 else "moderate relationship"
