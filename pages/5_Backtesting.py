...
with st.sidebar:
    ...
    <div style="padding:1.2rem 0.4rem 0.4rem">
        ...
        <div class="nav-item active">Backtesting</div>
    </div>
...
def generate_signals(df: pd.DataFrame, strategy: str) -> pd.Series:
    ...
    if strategy == "SMA Crossover (20/50)":
        ...
        sig[(df["MA20v50_x"] == 1) & (prev == 0)] =  1
        sig[(df["MA20v50_x"] == 0) & (prev == 1)] = -1
        ...
        prev_macd = df["MACD"].shift(1);  prev_s = df["Signal"].shift(1)
...
def compute_metrics(equity: pd.Series, capital: float, trades: list, df: pd.DataFrame):
    ...
    max_drawdown = float(((equity - equity.cummax()) / equity.cummax() * 100).min())
    ...
    return dict(
        ...
        max_drawdown=max_drawdown, win_rate=win_rate, avg_win=avg_win, avg_loss=avg_loss,
        profit_factor=profit_factor, total_trades=len(sell_trades),
        wins=len(wins), losses=len(losses), final_equity=float(equity.iloc[-1]),
    )
...
if not run_btn and "bt_result" not in st.session_state:
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:20px;
        ...
        <div style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:700;
            color:#f0f2f8;margin-bottom:.4rem">Configure Your Strategy</div>
        <div style="font-size:.83rem;color:#8892a4">
            Pick a stock, strategy, and period in the sidebar,
            then click <strong style="color:#4f8fff">Run Backtest</strong>.
        </div>
...
for col, (title, val, positive) in zip(kc2, [
    ("Win Rate",      f"{m['win_rate']:.1f}%",  m['win_rate'] >= 50),
    ("Profit Factor", pf_display,                m['profit_factor'] >= 1),
    ...
    with col:
        st.markdown(
            ...
            f'<div class="sub">W:{m["wins"]} / L:{m["losses"]}</div></div>',
            unsafe_allow_html=True,
        )
