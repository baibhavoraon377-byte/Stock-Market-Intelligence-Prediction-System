...
def save():
    ...
    with open(PORTFOLIO_FILE,"w") as f: json.dump(st.session_state.portfolio,f,indent=2)

def load():
    if os.path.exists(PORTFOLIO_FILE):
        with open(PORTFOLIO_FILE) as f: st.session_state.portfolio = json.load(f)
load()
...
def calc(holdings):
    ...
    for h in holdings:
        ...
        tv+=cur; ti+=inv
    for r in rows: r["allocation"]=(r["current_value"]/tv*100) if tv else 0
...
with st.sidebar:
    ...
    with st.form("add"):
        ...
        if st.form_submit_button("➕ Add",use_container_width=True) and sym and shrs>0 and bp>0:
            ...
            save(); st.success(f"Added {shrs}×{sym.upper()}"); st.rerun()
...
# ── Header ─────────────────────────────────────────────────────────────────────
st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:1.6rem;font-weight:800;color:#f0f2f8;padding:.6rem 0 .3rem">Portfolio Tracker</div><div style="font-size:.82rem;color:#8892a4">Real-time P&amp;L · Allocation · Benchmark comparison · CSV/Excel export</div>',unsafe_allow_html=True)

holdings = st.session_state.portfolio["holdings"]

if not holdings:
    st.markdown("""<div style="background:#13161e;border:1px dashed rgba(255,255,255,.12);border-radius:20px;padding:3rem 2rem;text-align:center;margin-top:1rem">
...
for col,lbl,val,color in zip(st.columns(4),
    ...
    [f"${tv:,.2f}",
     f"{'▲' if total_pnl>=0 else '▼'} ${abs(total_pnl):,.2f} ({total_pnl_pct:+.2f}%)",
     f"${total_inv:,.2f}",
     f"${st.session_state.portfolio['cash_balance']:,.2f}"],
    ["#4f8fff",pnl_color,"#f5a623","#22d98a"]):
    with col:
        st.markdown(f'<div class="stat-card"><div class="stat-label">{lbl}</div><div class="stat-value" style="color:{color};font-size:1.5rem">{val}</div></div>',unsafe_allow_html=True)

st.markdown("<div style='height:.6rem'></div>",unsafe_allow_html=True)

tab1,tab2,tab3,tab4 = st.tabs(["📊 Holdings","📈 Performance","⚠️ Risk","📋 Reports & Export"])

# ── HOLDINGS ──────────────────────────────────────────────────────────────────
with tab1:
    c1,c2 = st.columns(2)
    with c1:
        alloc_df = pd.DataFrame([{"Symbol":m["symbol"],"Allocation":m["allocation"]} for m in metrics if m["allocation"]>0])
        if not alloc_df.empty:
            ...
            fig.update_traces(textfont=dict(size=10,color="#8892a4"),
    ...
    with c2:
        ...
        fig2 = go.Figure(go.Bar(x=perf_df.Symbol,y=perf_df["Return %"],marker_color=colors,marker_line_width=0))
...
with tab3:
    ...
    div_lbl = "Well diversified" if n>=7 else "Moderate" if n>=3 else "Concentrated — add more"
    conc_lbl= "High concentration ⚠️" if largest>40 else "Balanced ✅" if largest<25 else "Moderate"

    c1,c2,c3,c4 = st.columns(4)
    for col,lbl,val,sub in [(c1,"Holdings",str(n),div_lbl),
                             (c2,"Largest Position",f"{largest:.1f}%",conc_lbl),
                             (c3,"Cash Weight",f"{cash_w:.1f}%","Uninvested"),
                             (c4,"Stocks",f"{n}","Unique positions")]:
    ...
    if rets_list:
        ...
        for col,lbl,val in [(pm1,"Est. Portfolio Vol",f"{port_vol:.1f}%/yr"),
...
with tab4:
    ...
    with c1:
        ...
        df_export = pd.DataFrame([{
            ...
            "Current Value":round(m["current_value"],2),"P&L $":round(m["pnl"],2),
            "P&L %":round(m["pnl_pct"],2),"Allocation %":round(m["allocation"],2),
            "Buy Date":m.get("buy_date","—"),
        } for m in metrics])
        st.download_button("📥 Download Holdings CSV",df_export.to_csv(index=False).encode(),
                           file_name="portfolio_holdings.csv",mime="text/csv",use_container_width=True)
    ...
    with c2:
        ...
        with pd.ExcelWriter(buf,engine="openpyxl") as w:
            ...
            summary = pd.DataFrame([{
                ...
                "Total P&L":round(total_pnl,2),"P&L %":round(total_pnl_pct,2),
    ...
    st.markdown("<div style='height:.6rem'></div>",unsafe_allow_html=True)
    st.markdown("#### 📊 Holdings Summary Preview")
