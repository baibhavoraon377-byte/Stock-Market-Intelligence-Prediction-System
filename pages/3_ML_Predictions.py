...
def fetch_and_train(symbol: str, period: str, forecast_days: int, model_choice: str):
    ...
    for name, m in models.items():
        ...
        dacc = float(np.mean(np.sign(np.diff(p)) == np.sign(np.diff(y_test))) * 100)
    ...
    for name, m in models.items():
        ...
        for _ in range(forecast_days):
            ...
            preds.append(next_p)
            row[0, 0] = (next_p - row[0, -1]) / max(row[0, -1], 1e-9)
    ...
    return dict(
        history=df, feature_cols=feature_cols, y_test=y_test,
        preds_test=preds_test, metrics=metrics, future_preds=future_preds,
        ensemble_future=ensemble_future, last_close=float(df["Close"].iloc[-1]),
