...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Multiple windows of MA, STD, momentum, ROC
    for w in [5, 10, 20, 50]:
    ...
    # Price ratios to moving averages (mean-reversion features)
    df["PriceMA5_ratio"]  = df["Close"] / df["MA5"].replace(0, np.nan)
    df["PriceMA20_ratio"] = df["Close"] / df["MA20"].replace(0, np.nan)
    # RSI
    delta = df["Close"].diff()
    ...
    # Volume ratio
    df["VolRatio"] = df["Volume"] / df["VMA10"].replace(0, np.nan)
...
def fetch_and_train(symbol: str, period: str, forecast_days: int, model_choice: str):
    ...
    for name, m in models.items():
        if name == "LSTM":
            ...
            p = _predict_lstm(lstm_model, lstm_type, X_seq_te)
            y_ref = y_seq_te
        else:
    ...
    for name, m in models.items():
        if name == "LSTM":
            ...
            for _ in range(forecast_days):
                ...
                next_p    = float(_predict_lstm(lstm_model, lstm_type, inp)[0])
                preds.append(next_p)
                # Roll the window: drop oldest, append new row (shift Close proxy)
            ...
            preds = []
            for _ in range(forecast_days):
                next_p = float(m.predict(row)[0])
                preds.append(next_p)
                row = row.copy()  # features stay constant (price proxy)
...
if not run_btn and "ml_result" not in st.session_state:
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:20px;
        ...
        <div style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:700;
            color:#f0f2f8;margin-bottom:.4rem">Configure &amp; Run a Prediction</div>
        <div style="font-size:.83rem;color:#8892a4">
            Select a stock, training period, and model in the sidebar,
            then click <strong style="color:#4f8fff">Run Prediction</strong>.
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
