...
# Extra compliance-specific CSS (minimal — only what's not in shared)
st.markdown("""
<style>
.compliance-card{
...
.v-tag{display:inline-flex;align-items:center;gap:3px;font-size:.65rem;font-weight:600;padding:3px 8px;border-radius:6px;}
...
.nav-badge{background:var(--accent-red);color:white;font-size:.62rem;font-weight:700;padding:2px 7px;border-radius:20px;}
...
recent_prompts = [
    ...
    {"user": "Alex Lee",    "dept": "Legal",       "avatar": "AL",
     "summary": "Analyze this attached unredacted contract…",
...
with st.sidebar:
    ...
    <div style="padding:1.2rem .4rem .6rem">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:1.2rem">
            ...
            <div>
                <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:.95rem;
                    color:#f0f2f8!important">AI Compliance</div>
        ...
        <div class="nav-item" style="justify-content:space-between">
            <span>⚠️ Violations</span><span class="nav-badge">12</span></div>
...
with hc1:
    ...
    <div style="padding:.8rem 0 .2rem">
        <div style="font-family:'Syne',sans-serif;font-size:1.7rem;font-weight:800;
            color:#f0f2f8;line-height:1">AI Compliance Overview</div>
        <div style="font-size:.82rem;color:#8892a4;margin-top:.3rem">
            Monitor and manage AI workflow risks across your organisation</div>
    </div>
    """, unsafe_allow_html=True)
with hc2:
    st.markdown(f"""
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;
        ...
        <div style="font-size:.62rem;color:#4e5669;text-transform:uppercase;letter-spacing:.07em">Live Status</div>
        <div style="font-size:.78rem;font-weight:600;color:#f0f2f8">{datetime.now().strftime("%b %d  –  %H:%M")}</div>
    </div>
    """, unsafe_allow_html=True)
...
for col, val, lbl, badge_text, badge_cls, icon in [
    (sc1, str(total_interactions), "Total AI interactions",       "▲ 12% vs last week", "badge-up",   "👥"),
    (sc2, str(total_flags),        "Flagged by policy engine",    "▼ 5% improving",     "badge-up",   "🚩"),
    (sc3, "94/100",                "Org-wide compliance score",   "▲ Good standing",    "badge-up",   "🛡️"),
    (sc4, str(pending_reviews),    "High-risk issues pending",    "Needs attention",    "badge-down", "⚠️"),
]:
    with col:
        st.markdown(f"""
        <div class="compliance-card">
            <div style="display:flex;justify-content:space-between;align-items:flex-start">
                ...
                <span class="stat-badge {badge_cls}">{badge_text}</span>
            </div>
...
with cc1:
    st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:.9rem;font-weight:700;'
                'color:#f0f2f8;margin-bottom:.2rem">AI Prompt Volume (7 Days)</div>',
                unsafe_allow_html=True)
    st.markdown('<div style="font-size:.75rem;color:#4e5669;margin-bottom:.8rem">'
                'Total prompts vs flagged high-risk prompts</div>', unsafe_allow_html=True)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dates, y=total_p, name="Total Prompts",
...
with cc2:
    st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:.9rem;font-weight:700;'
                'color:#f0f2f8;margin-bottom:.2rem">Risk by Department</div>', unsafe_allow_html=True)
    st.markdown('<div style="display:inline-block;font-size:.65rem;font-weight:600;padding:2px 8px;'
                'border-radius:6px;background:rgba(240,82,82,.15);color:#f05252;margin-bottom:.8rem">'
                'Top Risk: Engineering</div>', unsafe_allow_html=True)

    st.markdown('<div class="tbl-hdr" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr">'
                '<span>DEPT</span><span>LOW</span><span>MED</span><span>HIGH</span></div>',
                unsafe_allow_html=True)
...
<div style="display:grid;grid-template-columns:2.5fr 3fr 1.5fr 1.5fr 2fr 1fr;
    ...
    <span>USER</span><span>PROMPT SUMMARY</span><span>VIOLATION</span>
...
for p in recent_prompts:
    ...
    <div style="display:grid;grid-template-columns:2.5fr 3fr 1.5fr 1.5fr 2fr 1fr;
        ...
        border-left:2px solid {border_color};font-size:.8rem">
        ...
        <div style="color:#8892a4;font-size:.78rem;overflow:hidden;text-overflow:ellipsis;
            white-space:nowrap;padding-right:.5rem">{p['summary']}</div>
        <div><span class="v-tag {p['vtag']}">{p['vtype']}</span></div>
        <div style="color:#8892a4;font-size:.78rem">{p['model']}</div>
        <div style="color:#4e5669;font-size:.75rem">{p['time']}</div>
        <div><button style="background:rgba(28,32,48,1);border:1px solid rgba(255,255,255,.14);
...
with bc1:
    st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:.85rem;font-weight:700;'
                'color:#f0f2f8;margin-bottom:.5rem">AI Model Usage Distribution</div>',
                unsafe_allow_html=True)
...
with bc2:
    st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:.85rem;font-weight:700;'
                'color:#f0f2f8;margin-bottom:.5rem">Weekly Violation Trend</div>',
                unsafe_allow_html=True)
    weeks      = ["Week 1", "Week 2", "Week 3", "Week 4"]
    violations = [45, 38, 32, 28]
    fig3 = go.Figure()
    fig3.add_trace(go.Scatter(
        x=weeks, y=violations, mode="lines+markers",
        line=dict(color="#f05252", width=2.5),
        marker=dict(size=7, color="#f05252", line=dict(color="#0d0f14", width=2)),
        fill="tozeroy", fillcolor="rgba(240,82,82,.07)",
    ))
    # Trend label
    improvement = violations[0] - violations[-1]
    fig3.update_layout(
        height=280, yaxis_title="Violations",
        title=f"↓ {improvement} violations reduced this month",
        title_font=dict(color="#22d98a", size=11), **DARK_LAYOUT,
    )
...
<div style="text-align:center;padding:1.2rem 0 .3rem;font-size:.72rem;color:#4e5669">
    🛡️ AI Compliance Dashboard · Real-time monitoring active ·
