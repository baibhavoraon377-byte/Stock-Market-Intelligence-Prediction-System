...
.watch-row{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-md);
    padding:1rem 1.2rem;margin-bottom:.5rem;transition:border-color .2s;}
...
with st.sidebar:
    ...
    if st.button("Add Selected", use_container_width=True, key="wl_add_sel"):
    ...
    new_sym = st.text_input("Or enter symbol", placeholder="e.g. GOOG, BTC-USD",
                             label_visibility="collapsed", key="wl_custom")
    if st.button("Add Custom", use_container_width=True, key="wl_add_cust"):
    ...
    al_custom   = st.text_input("Or custom symbol", placeholder="AAPL", key="al_sym",
                                 label_visibility="collapsed")

    if al_custom.strip():
        alert_sym = al_custom.strip().upper()
    elif al_chosen != "— choose —":
        alert_sym = STOCK_CATALOGUE[al_chosen]
    else:
        alert_sym = ""
    ...
    if st.button("Set Alert", use_container_width=True, type="primary"):
        if alert_sym:
            ...
            st.warning("Please select or enter a symbol first.")
