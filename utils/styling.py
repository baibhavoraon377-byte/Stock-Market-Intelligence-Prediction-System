"""
utils/styling.py
Single source of truth for CSS + Plotly defaults.

SIDEBAR TOGGLE FIX
------------------
The original code used  #MainMenu,footer,header { visibility:hidden }
which buried the hamburger / sidebar-toggle inside <header>, making it
impossible to reopen the sidebar after closing it.

Fix applied here:
  1. We never hide <header> — only footer + deploy clutter.
  2. We force [data-testid="collapsedControl"] to be fixed-position,
     z-index 9999, always visible and interactive even when sidebar is closed.
"""
...
/* ── HIDE CLUTTER ONLY — never hide the header that holds the toggle ── */
footer                         { visibility:hidden!important; }
[data-testid="stDeployButton"] { display:none!important; }
[data-testid="stDecoration"]   { display:none!important; }
[data-testid="stStatusWidget"] { display:none!important; }

/* ── SIDEBAR TOGGLE: fixed so it is always reachable ── */
...
[data-testid="collapsedControl"] svg {
    color:var(--text-secondary)!important; fill:var(--text-secondary)!important;
}
[data-testid="collapsedControl"]:hover svg { color:#fff!important; fill:#fff!important; }
...
section[data-testid="stSidebar"] > div:first-child { padding-top:.8rem!important; }
...
div[data-testid="stMetric"] [data-testid="stMetricValue"] { font-family:'Syne',sans-serif!important; color:var(--text-primary)!important; font-size:1.5rem!important; }
...
.p-stat { background:var(--bg-card); border:1px solid var(--border); border-radius:var(--radius-lg); padding:1.3rem 1.5rem; position:relative; overflow:hidden; }
.p-stat::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,var(--accent-blue),var(--accent-violet)); opacity:.5; }
...
# Legacy compat shims
class ColorPalette:
    BLUE = "#4f8fff"; VIOLET = "#7c6ff7"; GREEN = "#22d98a"
    RED  = "#f05252"; AMBER  = "#f5a623"; BG_BASE = "#0d0f14"; BG_CARD = "#13161e"

class DashboardStyling:
    @staticmethod
    def apply_custom_css(): apply_css()

def init_styling(): apply_css()
