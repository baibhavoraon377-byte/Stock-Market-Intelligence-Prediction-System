...
SIDEBAR FIX NOTES
-----------------
The collapse-toggle button lives inside the Streamlit <header> element.
Previous attempts hid the entire header via  header { visibility:hidden }
which also buried the toggle button.

The correct approach used here:
  1. Never hide <header> or #MainMenu.
  2. Hide only the clutter (footer text, deploy button, decoration bar).
  3. Target [data-testid="collapsedControl"] explicitly to keep it
     visible, positioned, and styled even when the sidebar is closed.
  4. Give the sidebar a min-width so it never fully disappears.
"""
...
/* ── HIDE CLUTTER — NOT THE HEADER THAT HOLDS THE TOGGLE ── */
...
/* This button appears when the sidebar is collapsed.
   We force it to remain rendered, visible, and above all other content. */
...
.bt-card { background:var(--bg-card); border:1px solid var(--border); border-radius:var(--radius-md); padding:1rem 1.2rem; }
...
.nav-badge { background:var(--accent-red); color:white; font-size:.62rem; font-weight:700; padding:2px 7px; border-radius:20px; margin-left:auto; }
...
.screener-row { align-items:center; padding:.55rem .8rem; border-bottom:1px solid rgba(255,255,255,.04); font-size:.8rem; transition:background .12s; }
.screener-row:hover { background:rgba(255,255,255,.03); }
