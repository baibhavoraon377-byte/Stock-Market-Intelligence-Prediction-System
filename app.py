...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Add MA, RSI, Bollinger Bands and MACD columns to a price DataFrame."""
    df = df.copy()
    ...
    # FIX: avoid ZeroDivisionError when loss is 0
...
# FIX: convert list → tuple so @st.cache_data can hash it
...
if selected_symbol and selected_symbol in stocks_data:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Price Chart", "📊 Indicators", "ℹ️ Company Info", "🔔 Alerts"]
    )
    ...
    with tab1:
        ...
        fig = make_subplots(
            ...
            vertical_spacing=0.18, row_heights=[0.58, 0.21, 0.21],
        ...
        fig.update_layout(
            height=820, showlegend=True, xaxis_rangeslider_visible=False,
    ...
    with tab2:
        ...
        fig2.update_layout(
            height=340, title="Price vs Moving Averages",
