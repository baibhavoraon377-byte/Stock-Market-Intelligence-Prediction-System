...
# ── Page config ── ONLY here, never in pages/ ──────────────────────────────────
st.set_page_config(
    page_title="StockFin",
    page_icon="📈",
    layout="wide",
...
if "alerts" not in st.session_state:
    # list of {"symbol","condition","price","triggered"}
...
with st.sidebar:
    ...
    page = st.radio("Navigate", [
        ...
        "🔍 Screener", "📡 Watchlist & Alerts",
        "🗺️ Sector Heatmap", "🔬 Backtesting",
        "📅 Calendar & Earnings", "📋 Options Chain", "🛡️ Compliance",
...
if page == "🏠 Dashboard":
    ...
    for i,(sym,d) in enumerate(list(stocks.items())[:8]):
        with cols[i % len(cols)]:
            up = d["change"] >= 0
            st.markdown(f'<div class="ticker-card"><div class="ticker-sym">{sym}</div><div class="ticker-price">${d["price"]:.2f}</div><div class="ticker-chg {"up" if up else "down"}">{"▲" if up else "▼"} {abs(d["change_pct"]):.2f}%</div></div>', unsafe_allow_html=True)
    ...
    if sel:
        ...
        tab1,tab2,tab3,tab4 = st.tabs(["📈 Chart","📊 Indicators","ℹ️ Company","🔔 Alerts"])
        ...
        with tab1:
            ...
            for c,lbl,col,dash in [("MA20","MA20","#f5a623","solid"),("MA50","MA50","#7c6ff7","solid"),
                                    ("BB_U","BB+","rgba(255,255,255,.2)","dot"),("BB_L","BB−","rgba(255,255,255,.2)","dot")]:
                fig.add_trace(go.Scatter(x=hist.index,y=hist[c],name=lbl,line=dict(color=col,width=1,dash=dash)),row=1,col=1)
            bc = ["#22d98a" if hist.Close.iloc[i]>=hist.Open.iloc[i] else "#f05252" for i in range(len(hist))]
            fig.add_trace(go.Bar(x=hist.index,y=hist.Volume,name="Vol",marker_color=bc,opacity=.7),row=2,col=1)
            fig.add_trace(go.Scatter(x=hist.index,y=hist.RSI,name="RSI",line=dict(color="#4f8fff",width=1.5)),row=3,col=1)
        ...
        with tab4:
            ...
            if rsi_v > 70: alerts_found.append(("📊",f"RSI overbought at <strong>{rsi_v:.1f}</strong> — pullback zone","#f05252"))
            if rsi_v < 30: alerts_found.append(("📊",f"RSI oversold at <strong>{rsi_v:.1f}</strong> — bounce zone","#22d98a"))
            if avgvol and d["volume"] > avgvol*1.5: alerts_found.append(("📢",f"Volume spike: <strong>{d['volume']:,.0f}</strong> vs avg {avgvol:,.0f}","#4f8fff"))
            bbw = (float(hist.BB_U.iloc[-1])-float(hist.BB_L.iloc[-1]))/float(hist.BB_M.iloc[-1])
            if bbw < 0.05: alerts_found.append(("🎯","Bollinger squeeze — breakout approaching","#7c6ff7"))
            if alerts_found:
    ...
    for col,title,val,icon in zip(st.columns(4),
        ...
        [f"${est_ret:,.2f}",f"{avg_chg:+.2f}%",f"{gainers}↑  {losers}↓",str(len(stocks))],
        ["💰","📈","🎯","📋"]):
    ...
    with wl_col:
        ...
        for sym in st.session_state.watchlist:
            ...
            if d:
                ...
                with c2:
                    st.markdown(f'<div style="font-size:1.1rem;font-weight:700;color:#f0f2f8">${d["price"]:.2f}</div><div style="font-size:.75rem;color:{"#22d98a" if up else "#f05252"}">{"▲" if up else "▼"} {abs(d["change_pct"]):.2f}%</div>', unsafe_allow_html=True)
                with c3:
    ...
    with al_col:
        ...
        if st.session_state.alerts:
            for i, al in enumerate(st.session_state.alerts):
                ...
                st.markdown(f'<div class="alert-item" style="border-color:{color};border:1px solid {color}33;border-left:3px solid {color}">{icon} <strong style="color:#f0f2f8">{al["symbol"]}</strong> — {al["condition"]} <strong>${al["price"]:.2f}</strong>{"  ✅ TRIGGERED" if triggered else ""}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:1.6rem;font-weight:800;color:#f0f2f8;padding:.6rem 0 .3rem">Stock Screener</div>', unsafe_allow_html=True)

    UNIVERSE = ["AAPL","MSFT","GOOGL","AMZN","NVDA","META","TSLA","NFLX","AMD","INTC",
                "JPM","GS","BAC","WFC","V","MA","PYPL","SQ","COIN","HOOD",
                "JNJ","PFE","MRNA","ABBV","UNH","CVS","WMT","COST","TGT","HD"]

    fc1,fc2,fc3,fc4 = st.columns(4)
    with fc1: min_price,max_price = st.slider("Price Range ($)",0,5000,(0,5000))
    with fc2: min_chg,max_chg    = st.slider("Day Change %",-20,20,(-20,20))
    with fc3: min_vol             = st.number_input("Min Volume (M)",value=0.0,step=0.5)
    with fc4: sectors             = st.multiselect("Sector",["Technology","Healthcare","Finance","Consumer","Energy","Any"],default=["Any"])
    ...
    if st.button("🔍 Run Screener", type="primary"):
        with st.spinner(f"Scanning {len(UNIVERSE)} stocks…"):
            ...
            for i,sym in enumerate(UNIVERSE):
                ...
                if d:
                    ...
                    chg = d["change_pct"]
                    vol = d["volume"]/1e6
                    sec = d["info"].get("sector","Unknown")
                    ok  = (min_price<=p<=max_price and min_chg<=chg<=max_chg and vol>=min_vol)
                    if ok and ("Any" not in sectors):
                        ok = any(s.lower() in sec.lower() for s in sectors)
                    if ok:
                        results.append({"Symbol":sym,"Price":f"${p:.2f}","Change":f"{chg:+.2f}%",
                                        "Volume(M)":f"{vol:.1f}","Sector":sec,
                                        "52W High":f"${d['info'].get('fiftyTwoWeekHigh',0):.2f}",
                                        "P/E":d['info'].get('trailingPE','—')})
        ...
        if results:
            ...
            st.download_button("📥 Export Results (CSV)", buf.getvalue(),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_etf:
        ...
        if sector_data:
            ...
            for i,row in df_s.iterrows():
                ...
                if chg >= 0:
                    bg = f"rgba(34,217,138,{0.1+intensity*0.4})"
                    tc = "#22d98a"
                else:
                    bg = f"rgba(240,82,82,{0.1+intensity*0.4})"
                    tc = "#f05252"
    ...
    with tab_stocks:
        ...
        if sdata:
            ...
            sizes  = [max(abs(d["price"])*10,200) for _,d in sdata]
            fig2 = go.Figure(go.Treemap(
                ...
                marker=dict(colors=values,colorscale=[[0,"#f05252"],[0.5,"#1c2030"],[1,"#22d98a"]],
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("▶ Run Backtest", type="primary"):
        with st.spinner("Running backtest…"):
            ...
            st.markdown(f'<div style="margin:.5rem 0;font-size:.8rem;color:#8892a4">Trades executed: <strong style="color:#f0f2f8">{n_trades}</strong> &nbsp;·&nbsp; Alpha vs B&H: <strong style="color:{"#22d98a" if total_r>bh_r else "#f05252"}">{total_r-bh_r:+.1f}%</strong></div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE: CALENDAR & EARNINGS
# ══════════════════════════════════════════════════════════════════════════════
elif page == "📅 Calendar & Earnings":
    st.markdown('<div style="font-family:\'Syne\',sans-serif;font-size:1.6rem;font-weight:800;color:#f0f2f8;padding:.6rem 0 .3rem">Economic Calendar &amp; Earnings Dates</div>', unsafe_allow_html=True)

    tab_earn, tab_cal = st.tabs(["📊 Earnings Dates","📅 Economic Events"])

    with tab_earn:
        ec1,ec2 = st.columns([2,1])
        with ec1: earn_sym = st.text_input("Stock Symbol","AAPL",key="earn_sym")
        with ec2: st.markdown("<div style='height:1.8rem'></div>",unsafe_allow_html=True)

        if st.button("Fetch Earnings Info"):
            with st.spinner("Fetching…"):
                try:
                    t = yf.Ticker(earn_sym.upper())
                    cal = t.calendar
                    info= t.info
                    ...
                    if cal is not None and not (isinstance(cal,dict) and len(cal)==0):
                        if isinstance(cal, pd.DataFrame):
                            ...
                            for k,v in cal.items():
                                st.markdown(f'<div style="padding:.3rem 0;font-size:.83rem;color:#8892a4"><strong style="color:#f0f2f8">{k}:</strong> {v}</div>', unsafe_allow_html=True)
                    ...
                    for i,(k,v) in enumerate(items_list):
                        with (c1 if i<len(items_list)//2 else c2):
                            st.markdown(f'<div style="padding:.28rem 0;border-bottom:1px solid rgba(255,255,255,.04);font-size:.82rem"><span style="color:#8892a4">{k}</span><span style="float:right;color:#f0f2f8;font-weight:500">{v}</span></div>', unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"Error: {e}")

    with tab_cal:
        st.markdown("#### 📅 Key Economic Events (Sample)")
        events = [
            {"Date":"2025-04-30","Event":"Fed FOMC Meeting","Impact":"🔴 High","Expected":"Rate hold 4.25-4.5%"},
            {"Date":"2025-05-02","Event":"US Non-Farm Payrolls","Impact":"🔴 High","Expected":"185K jobs"},
            {"Date":"2025-05-13","Event":"US CPI Inflation","Impact":"🔴 High","Expected":"3.1% YoY"},
            {"Date":"2025-05-15","Event":"US Retail Sales","Impact":"🟡 Medium","Expected":"+0.3% MoM"},
            {"Date":"2025-05-22","Event":"Fed Minutes Release","Impact":"🟡 Medium","Expected":"Hawkish tone"},
            {"Date":"2025-06-11","Event":"Fed FOMC Meeting","Impact":"🔴 High","Expected":"Rate decision"},
            {"Date":"2025-06-25","Event":"US GDP Q1 Final","Impact":"🟡 Medium","Expected":"+2.1% annualised"},
        ]
        df_ev = pd.DataFrame(events)
        st.dataframe(df_ev,use_container_width=True,hide_index=True)
        st.markdown('<div class="insight-text" style="margin-top:.6rem">💡 Economic events are sample placeholders. For live data, connect a financial calendar API (e.g. Finnhub, Alpha Vantage).</div>', unsafe_allow_html=True)
    ...
    if st.button("📋 Load Options Chain", type="primary"):
        with st.spinner("Fetching options…"):
            try:
                ...
                def style_chain(df_opt: pd.DataFrame, is_call: bool) -> pd.DataFrame:
                    ...
                    df_opt["inTheMoney"] = df_opt.get("inTheMoney",False).map({True:"✅",False:"—"})
                    return df_opt

                if opt_type in ("Calls","Both"):
                    st.markdown("#### 📗 Calls")
                    calls_df = style_chain(chain.calls, True)
                    st.dataframe(calls_df.style.apply(
                        lambda r: ["background:rgba(34,217,138,.07)"]*len(r) if r.get("inTheMoney","")=="✅" else [""]*len(r),
                        axis=1), use_container_width=True, hide_index=True)

                if opt_type in ("Puts","Both"):
                    st.markdown("#### 📕 Puts")
                    puts_df = style_chain(chain.puts, False)
                    st.dataframe(puts_df.style.apply(
                        lambda r: ["background:rgba(240,82,82,.07)"]*len(r) if r.get("inTheMoney","")=="✅" else [""]*len(r),
                        axis=1), use_container_width=True, hide_index=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# Remaining pages delegate to sub-files
# ══════════════════════════════════════════════════════════════════════════════
elif page in ("📊 Analytics","💼 Portfolio","🛡️ Compliance"):
    st.info(f"**{page}** is available as a dedicated page in the `pages/` folder. Navigate using Streamlit's built-in page menu, or select it from the sidebar.")
