...
# ── Custom overlay sidebar CSS + animation ───────────────────────────────────
st.markdown("""
<style>
...
.sf-brand-name {
    font-family: 'Syne', sans-serif;
    font-weight: 700; font-size: 1rem; color: #f0f2f8;
}
.sf-close-btn {
...
.sf-close-btn:hover { background: rgba(240,82,82,0.15); color: #f05252; }
...
.sf-nav-item:hover { background: rgba(255,255,255,0.05); color: #d0d6e8; }
.sf-nav-item.active {
...
.sf-live {
    ...
    font-size: .62rem; font-weight: 600; color: #22d98a;
    margin-top: 2px;
}
.sf-live::before {
    content: '';
...
# ── Sidebar open/close state logic ───────────────────────────────────────────
sidebar_is_open = st.session_state.sidebar_open
open_class = "open" if sidebar_is_open else ""

# ── Render the custom overlay sidebar HTML ────────────────────────────────────
st.markdown(f"""
<!-- Backdrop -->
<div id="sf-backdrop" class="{open_class}" onclick="sfClose()"></div>

<!-- Sidebar panel -->
<div id="sf-sidebar" class="{open_class}">
  ...
  <div class="sf-header">
    <div class="sf-logo">
      ...
      <div>
        <div class="sf-brand-name">StockFin</div>
        <div class="sf-live">LIVE</div>
      </div>
    </div>
    <button class="sf-close-btn" onclick="sfClose()" title="Close">✕</button>
  </div>

  <!-- Platform nav -->
  <div class="sf-section-label">Platform</div>
  ...
  <!-- Pages nav -->
  <div class="sf-section-label">Pages</div>
  ...
  <!-- Settings nav -->
  <div class="sf-section-label">Settings</div>
  <div class="sf-nav-item">
    <span class="sf-icon">
  ...
  <div class="sf-nav-item">
    <span class="sf-icon">
      <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/></svg>
    </span>
    Account
  </div>
  ...
  <!-- Footer -->
  <div class="sf-footer">
    📡 Data · Yahoo Finance · ~60s refresh
  </div>
</div>

<!-- Toggle hamburger button -->
<button id="sf-toggle-btn" onclick="sfToggle()" title="Toggle menu">
...
function sfOpen() {{
    document.getElementById('sf-sidebar').classList.add('open');
    document.getElementById('sf-backdrop').classList.add('open');
}}
function sfToggle() {{
    const s = document.getElementById('sf-sidebar');
    if (s.classList.contains('open')) sfClose(); else sfOpen();
}}
// Re-apply open state on Streamlit reruns
(function() {{
    const shouldOpen = {'true' if sidebar_is_open else 'false'};
    if (shouldOpen) sfOpen();
}})();
</script>
""", unsafe_allow_html=True)
...
# ── Stock controls (hidden expander — acts as "sidebar" settings panel) ───────
st.markdown("""
<style>
/* Style the controls expander to feel like a settings panel */
...
with st.expander("⚙️  Stock Selection & Settings", expanded=True):
    st.markdown(
        '<div style="font-size:.7rem;font-weight:600;letter-spacing:.1em;'
        'text-transform:uppercase;color:#4e5669;padding:.1rem 0 .4rem">Select Stocks</div>',
        unsafe_allow_html=True,
    )
    ...
    for idx, (name, sym) in enumerate(STOCK_CATALOGUE.items()):
        if idx >= 12:
            break
        with check_cols[idx % 2]:
            if st.checkbox(name, value=(sym in ["AAPL", "TSLA", "MSFT"]), key=f"chk_{sym}"):
                selected_stocks.append(sym)

    st.markdown('<hr style="border-color:rgba(255,255,255,.06);margin:.6rem 0">', unsafe_allow_html=True)
    col_cp, col_inv = st.columns(2)
    with col_cp:
